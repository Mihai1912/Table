...
case class Field(colName: String, predicate: String => Boolean) extends FilterCond {
  ...
  override def eval(r: Row): Option[Boolean] = {
    if (!r.contains(colName)) {
      ...
      Some(predicate.apply(r(colName)))
    }
  }
}
...
class Table(columnNames: Line, tabular: List[List[String]]) {
  ...
  def filter(cond: FilterCond): Option[Table] = {
    ...
    val a = rowList.map(row => if (cond.eval(row).isEmpty) return None else if (cond.eval(row).get) row else Nil).filter(_ != Nil)
    val b = a.map(elem => getColumnNames.map(name => elem.toMap.get(name).get))
    Some(new Table(getColumnNames, b))
  }
